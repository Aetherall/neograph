...
local function render_item(item, entity)
    ...
    -- BISECT: Thread + Frame + Scope + Variable property access
    local text = item.type .. " id=" .. item.id
    -- BISECT: Stack N signal:get() calls on every item
    local N = 4
    for _ = 1, N do
        _ = entity.name:get()
    end
    return { text = text, hl = nil, id = item.id }
end
...
local function render()
    ...
    -- BISECT: Full render with highlights
    ...
    -- BISECT: Skip highlight application
